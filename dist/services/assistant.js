 ...
 */
function removeTaskIds(reply, knownIds = []) {
    ...
    const idWithLabel = new RegExp(String.raw `\s*[(\[]?\s*(?:\b(?:task|event)\s+)?(?:\b(?:id|identifier)\b\s*(?:is|:|=|#)?\s*)?[\x60*_"']*(?:${idAlternatives})[\x60*_"']*\s*[)\]]?`, "gi");
    const cleanSentence = (sentence) => sentence
        ...
        .replace(/\s*[—–:,-]+\s*([.!?]?)$/, "$1")
        .trim();
    const isOnlyFiller = (sentence) => sentence
        .toLowerCase()
        .replace(/’/g, "'")
        .replace(/[*_\x60"()[\].,!?;:—–-]/g, " ")
        .split(/\s+/)
    ...
    const lines = reply.split("\n").flatMap((line) => {
        ...
        if (!anyId.test(line))
            return [line];
        const prefix = (_b = (_a = line.match(/^\s*(?:[-*+]\s+|\d+[.)]\s+)?/)) === null || _a === void 0 ? void 0 : _a[0]) !== null && _b !== void 0 ? _b : "";
        const kept = line
            .slice(prefix.length)
            .split(/(?<=[.!?])\s+/)
            .flatMap((sentence) => {
            if (!anyId.test(sentence))
                return [sentence];
...
function rememberTaskIds(value, ids, depth = 0) {
    ...
    for (const [key, item] of Object.entries(value)) {
        ...
        if (key === "id" && typeof item === "string")
            ...
            rememberTaskIds(item, ids, depth + 1);
    }
}
...
function runAssistant(options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const result = (reply) => ({
            reply: removeTaskIds(reply, state.knownIds) ||
                summarizeChanges() ||
